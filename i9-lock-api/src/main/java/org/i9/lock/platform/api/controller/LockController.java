...
public class LockController {
    ...
    public HashMap<String, Object> saveLock(LockAddDto lockAddDto, BindingResult bindingResult,
            @RequestParam(value = "uploadHead", required = false) MultipartFile uploadFile, HttpServletRequest request,
    ...
    public HashMap<String, Object> release(Lock lock) {
        ...
        HashMap<String, Object> result = new HashMap<String, Object>();
        // 1.更新锁的disturb
        Lock lockById = lockService.getLockById(lock.getId());
        lockById.setShowType(1);
        lockById.setDisturb(0);
        lockById.setMac(null);
        lockById.setKeyAdmin(null);
        lockById.setKeyDev(null);
        lockById.setKeyUser(null);
        // 删除mac，删除keyAdmin.keyNumber,keyDev,keyUser
        lockService.updateById(lockById);
        // 2.删除用户的长密码
     ...
     */
    @ResponseBody
    @RequestMapping("/checkLockMac")
    public HashMap<String, Object> checkLockMac(String mac) {
        HashMap<String, Object> result = new HashMap<String, Object>();
        LockExample lockExample = new LockExample();
        Criteria criteria = lockExample.createCriteria();
        ...
        List<Lock> list = lockService.selectByExample(lockExample);
        if(list.size()>0) {
            throw new BusinessException("mac地址已被占用！");
        }
        return result;
    }
}
